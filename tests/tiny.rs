...
fn tiny_97() {
	...
	// This PE file uses the fact that file mapping fills the remainder of the virtual memory page with zeroes
	// allowing it to trim the remainder of the optional headers.
	//
	// FileMap isn't clever enough and returns a slice of exact file size, so the validation will error with OOB.
	assert!(match file { Err(e) if e == pelite::Error::OOB => true, _ => false });
}
...
fn tiny_import_133() {
	...
	// Same issue as tiny.97, end of data directory falls in the zero filled bytes
	assert!(match file { Err(pelite::Error::OOB) => true, _ => false });
}
